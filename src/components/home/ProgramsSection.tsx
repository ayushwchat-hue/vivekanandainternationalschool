...
const programIcons = [Baby, Backpack, BookOpen, GraduationCap];
const colorClasses = ['secondary', 'primary', 'accent', 'secondary'];
...
const ProgramsSection = () => {
  ...
  const { content } = useSiteContent('programs');
  const extraData = getExtraData<ProgramsExtraData>(content, { programs: defaultPrograms });
  const programs = extraData.programs || defaultPrograms;
      ...
      <div className="container mx-auto px-4">
        ...
        >
          <div className="inline-flex items-center gap-2 bg-primary/10 rounded-full px-4 py-2 mb-4">
            <span className="text-sm font-medium text-primary">{content?.subtitle || 'Academic Programs'}</span>
          </div>
          <h2 className="font-display text-3xl md:text-4xl lg:text-5xl font-bold text-foreground mb-4">
            {content?.title || 'Programs for Every Stage'}
          </h2>
          <p className="text-muted-foreground text-lg">
            {content?.description || 'From early childhood to secondary education, we offer comprehensive programs designed to meet the unique needs of each developmental stage.'}
          </p>
        </div>
        ...
        <div ref={cardsRef} className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
          {programs.map((program, index) => {
            ...
            const color = colorClasses[index] || 'primary';
            ...
            return (
              ...
              >
                <div className={`w-14 h-14 rounded-xl ${colorClassMap[color]} flex items-center justify-center mb-4`}>
